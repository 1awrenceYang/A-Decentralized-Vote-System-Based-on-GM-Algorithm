...
void Encryption(int m, epoint* pk,epoint *G,epoint *OutC1,epoint*OutC2,uint32_t *OutC3)//加密前，一定要要设置Active Curve
{
    ...
    cout << "Public Key Validity Check" << endl;
    try
    {
    ...
    //epoint_set()
    ecurve_mult(h, S, S);
    cout<<"Public Key Validity Check Pass,encryption parameters init succeed!"<<endl;
    try
    {
   ...
   cout<<"Start C1 calculatioin"<<endl;
    /*********************************C1密文计算**************************************************/
    ...
    //epoint_print(c1);
    /*********************************C1密文计算**************************************************/
    cout<<"C1 calculation complete"<<endl;
    ...
    //epoint_print(X2Y2);
    cout<<"Start C2 calculatioin"<<endl;
    /*********************************C2密文计算**************************************************/
    if (m == 0)
    {
    ...
    //epoint_print(c2);
    /*********************************C2密文计算**************************************************/
    cout<<"C2 calculation complete"<<endl;
    epoint_free(ToxicWaste);
    cout<<"Start C3 calculatioin"<<endl;
    /*********************************C3密文计算**************************************************/
    for (int i = 0; i < 32; i++)
        HashInput[i] = X2[i];
    HashInput[32] = 1;
    ...
    /*********************************C3密文计算**************************************************/
    cout<<"C3 calculation complete"<<endl;
    /*********************************返回结果**************************************************/
    ...
    epoint_free(c2);
    epoint_free(X2Y2);
    cout<<"Heap Space Free Complete"<<endl;
}
int Decryption(epoint* c1, epoint* c2,epoint*G, uint32_t* c3, big sk)
{
    ...
    {
        cout<<"C1 or C2 is at infinity"<<endl;
        return -1;
    }
    big x = mirvar(0);
    big y = mirvar(0);
    ...
    if (!epoint_x(x))
    {
        cout<<"C1 is not on the curve"<<endl;
    ...
    if (epoint_comp(G, mG))
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            if (SM3_hash_result[i] != c3[i])
            {
                ...
                cout<<"Decryption failed for hash value error"<<endl;
    ...
    cout<<"Decryption Bad Parameters"<<enl;
...
void HomoEncryption(epoint* c1, epoint* c11, epoint* c2, epoint* c22,epoint*OutC1,epoint*OutC2)
{
    ...
    {
       cout<<"Homo Encryption failed for point not on curve"<<endl;
        throw PointNotOnCurve;
    }
    cout<<"Homo Encryption start"<<endl;
    epoint* temp1, * temp2;
    temp1 = epoint_init();
    temp2 = epoint_init();
    ...
    ecurve_add(c1, c11);
    ecurve_add(c2, c22);
    cout<<"Homo Encryption complete"<<endl;
...
void HomoDecryption(epoint* c1, epoint* c2, epoint* G , big sk , int *m)
{
    if (point_at_infinity(c1) || point_at_infinity(c2))
    {
        cout<<"Homo Decryption failed for point infinity"<<endl;
        throw PointInfinite;
    }
    big x1, x2, y1, y2, r;
    ...
    {
        cout<<"Homo Decryption failed for point not on curve"<<endl;
        throw PointNotOnCurve;
    }
    cout<<"Homo Decryption start"<<endl;
    ...
    for (int i = 0; i < MaxVoteNum; i++)
    {
        ...
        {
            cout<<"Homo Decryption complete"<<endl;
            *m = i;
            return;
        }
    }
    cout<<"Homo Decryption error for unknown reason"<<endl;
