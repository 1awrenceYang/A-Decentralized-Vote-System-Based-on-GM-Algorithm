...
big* GenPkbySecretShare(int n, int t, epoint*G,epoint *PkOut,big q)//输入总人数，参与秘密分享的人数，生成元G，输出一个公钥，以及秘密分享参数，用于解密时生成sk*C1
{
    ...
    for (int i = 0; i < n; i++)
    {
        RandomPolyParam[i] = GenPolyParam(t);//不可以释放
    }
